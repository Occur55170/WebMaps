...
export default {
  ...
  setup(props, { emit }){
    ...
    function LayerCheckBoxChange(e, item){
      ...
      let nestedSubNodeIndex = String(item.nestedSubNodeIndex)
          ? item.nestedSubNodeIndex
          : undefined
      let id =
          item.single_tiles || !e.target.selectedOptions
              ? item.id
    ...
    async function getTribes(){
      const result = await $.ajax(
          `https://blueprint.indigenoustribe.tw/api/tribes?${'years=' + state.conditionYear}&${'township=' + state.conditionTown}`,
          'GET',
      )
    ...
    function moveMap(params){
      if (params.target.value !== 'false'){
        let selectValue = state.selectLayerOption.find(
            (node) => node.tribeCode == params.target.value,
        )
    ...
    function isChecked(subNodeId){
      ...
      return props.currentLayers.some(
          (node) => node.id === subNodeId || node.id == anchorStr,
      )
    }
...
<template>
  <div class="condition bg-white">
    <div
        class="row mx-0 align-items-center flex-nowrap text-center p-2 fw-bold border-bottom"
    >
    ...
    <div class="py-3 px-4 content" style="max-height: 40vh; overflow-y: scroll">
      <div class="mb-2 landBoundary">
        <div v-for="(node, nodeIndex) in props.mapLayers" class="mb-2">
          <div
              class="title d-flex align-items-center fw-bold text-black order-1 mb-1 text-decoration-none"
          ...
          >
            ...
            <svg
                viewBox="0 0 24 24"
                :class="{ openTitle: state.DropDown == nodeIndex }"
            >
          ...
          >
            ...
            >
              ...
              <div
                  v-if="subNode.title === '新竹縣原住民部落範圍'"
                  class="w-100"
              >
                <select
                    class="me-2"
                    placeholder="請選擇年度"
                    v-if="props.tribeQuery['years'] !== undefined"
                    @change="tribeYear"
                >
                  <option
                      :value="item"
                      v-for="(item, key) in props.tribeQuery['years']"
                  >
                    {{ item }}
                  </option>
                </select>
                ...
                >
                  ...
                  <option
                      :value="item"
                      v-for="(item, key) in props.tribeQuery['township']"
                  >
                    {{ item }}
                  </option>
                </select>
                <select
                    class="me-2"
                    v-if="state.selectLayerOption.length !== 0"
                    @change="moveMap"
                >
                  <option :value="false">請選擇</option>
                  <option
                      :value="item.tribeCode"
                      v-for="(item, key) in state.selectLayerOption"
                  >
                    {{ item.tribeName }}
                  </option>
                </select>
            ...
            <div v-else>
              <div class="my-2">
                <input
                    type="checkbox"
                    :checked="
                    props.currentLayers.some((node) => {
                      let subNodeIds = mapList.getLayerIndex(node.id);
                      return (
                        subNodeIds.nodeIndex == nodeIndex &&
                        subNodeIds.subNodeIndex == subNodeIndex
                      );
                    })
                  "
                    @change="
              ...
              <div
                  class="d-flex flex-wrap px-2 pt-1"
                  style="background: #f4f4f4"
              >
                <span
                    class="me-2 d-flex flex-wrap align-items-center mb-1"
            ...
            >
              <a
                  ...
                  :href="subNode.legend"
                  target="_blank"
                  style="text-decoration: none; color: #a87777"
              >圖例連結</a
              >
            </div>
          </div>
