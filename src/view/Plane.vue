...
export default {
  ...
  setup(props, { emit }) {
    ...
    onMounted(async () => {
      await axios.get(`https://blueprint.indigenoustribe.tw/api/engineering_plan?tribeCode=${route.params?.id}`)
        .then(res => {
          state.planObj = res.data
        }).catch(FailMethod => {
  ...
  <div class="detail">
    ...
    <div class="p-4">
      <div class="d-flex align-items-center justify-content-between w-100">
        <h1 class="text-brown fw-bold d-flex align-items-center w-100" :class="state.mainTextColor">
            ...
            <div class="tbody w-100">
              <div class="d-flex shrink-0 flex-nowrap justify-content-between w-100"
                ...
                <div class="col-3 bg-grey-light">
                  <a href="" class="btn py-1 text-white bg-grey" target="_blank" @click.prevent="openLightBox(item)">
      ...
      <div class="d-flex justify-content-center">
        <a href="#" class="goBack btn bg-black text-white d-inline-block mb-4 cursor-pointer" @click.prevent="router.push({
          name: 'detail',
          params: {
            id: route.params?.id
          },
    ...
    <div class="lightWrap w-100 h-100 d-flex justify-content-center align-items-center" v-if="state.lightSwitch"
      @click.prevent="state.lightSwitch = false">
      <div class="p-4 rounded bg-white d-flex align-items-center" @click.stop>
        <span>密碼：</span>
        <div class="mx-2" style="width: 300px;" :class="{ 'error': state.loginFailed }">
          <input class="me-2 w-100 px-2 py-1" type="text" name="password" placeholder="請輸入下載密碼"
            v-model="state.currentText" @input="state.loginFailed = false">
        </div>
        <button class="btn bg-steel text-white" @click.prevent="downloadPassword()">輸入
        </button>
      </div>
    </div>
  </div>
