            ...
            switch (action){
                case 'layerMode':
                    if (value.checked){
                        ...
                        const haveInfoBox = state.layers[value.nodeIndex].group_layers[value.subNodeIndex].info_box !== null
                        ...
                        if (state[`${state.targetNum === 1 ? 'map1' : 'map2'}LayerStatus`].includes('3D')){
                            ...
                            const imageryLayers = ol3d.getCesiumScene().imageryLayers
                            // 給圖層加入layerId
                            const addEvent = imageryLayers.addImageryProvider(
