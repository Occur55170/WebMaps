  ...
  setup(props, { emit }){
      ...
      switch (action){
        case 'layerMode':
          if (value.checked){
            ...
            // 點選父層後，自動帶入子層群組
            let isSingleTiles =
                state.layers[value.nodeIndex].group_layers[value.subNodeIndex]
                    .single_tiles
            let haveInfoBox =
                state.layers[value.nodeIndex].group_layers[value.subNodeIndex]
                    .info_box !== null
            if (!isSingleTiles || haveInfoBox){
              ...
              layersAry.forEach((element) => {
                ...
                if (
                    element
                        .get('id')
                        .includes(
                            `node${value.nodeIndex}_subNode${value.subNodeIndex}_nestedSubNode`,
                        )
                ){
              ...
              value.id = getMapLayers.resetLayerId(
                  value.id,
                  'nestedSubNode',
                  state.selectValueTemp,
              )
            }
            // TODO: 有3D圖層的話，先移除 加入圖層後，再換回3D狀態
            if (
                state[
                    `${state.targetNum == 1 ? 'map1' : 'map2'}LayerStatus`
                    ].includes('3D')
            ){
              ol3d.setEnabled(false)
            }
            let targetLayer = getMapLayers.getLayer(
                state.layers[value.nodeIndex].group_layers[value.subNodeIndex],
                nestedSubNodeIndex,
                value.id,
            )
            ...
            if (
                ['雷達回波預測', '累積雨量預測', '氣溫預測'].includes(
                    targetLayer.get('label'),
                )
            ){
              const {
                currentLayerKey,
              ...
              state.temp[timeKey] = setInterval(function(){
                state.temp[`${timeKey}count`] =
                    state.temp[`${timeKey}count`] + 1 > 4
                        ? 0
                        : state.temp[`${timeKey}count`] + 1
            ...
            if (
                [
                  ...
                  '土石流潛勢溪流',
                  '落石分布',
                ].includes(targetLayer.get('label'))
            ){
            ...
            function removeLayersById(id){
              ...
              const toRemoveLayerId = layersAry.filter((element) =>
                  element?.get('id')?.includes(deleteKey),
              )
        ...
        case 'selectLayerMode':
          ...
          if (value.layerName === 'all'){
            let layersAry = targetLayers.getArray()
            let layersToRemove = layersAry.filter(
                (node) => !(node.get('id') === undefined),
            )
            layersToRemove.forEach((node) => {
              layerControl({
                ...
                value: {
                  checked: false,
                  ...getMapLayers.getLayerIndex(node.get('id')),
                },
              })
            })
          } else{
        ...
        case 'changeOrder':
          ...
          let {
            ...
            nestedSubNodeIndex,
              ...
              getMapLayers.getLayerIndex(value.id)
          let nowTileLayer = getMapLayers.getLayer(
              state.layers[nodeIndex].group_layers[subNodeIndex],
              nestedSubNodeIndex,
              value.id,
          )
        ...
        case 'baseMap':
          ...
          layersAry.forEach((element) => {
            if (
                element.get('type') == 'base' &&
                element.get('baseId') !== value.baseId
            ){
              target.removeLayer(element)
            }
            return true
    ...
    function onChangeDimensionMap(value){
      ...
      if (value === '3D'){
        ...
        const layerToRemove = layersArray.find((element) =>
            element.get('label').includes('近年歷史災害82處部落點位'),
        )
        if (layerToRemove){
        ...
        ol3d.setEnabled(false)
        state[`${ta}LayerStatus`] = state[`${ta}LayerStatus`].filter(
            (node) => node !== '3D',
        )
    ...
    function changeMapCount(qty){
      ...
      state.mapCount = qty
      let otherLayers = state[`${otherMap}LayerStatus`].filter(
          (node) => node !== '3D',
      )
      let otherLayersData = otherLayers.map((item) =>
          mapLayerList.getLayerIndex(item),
      )
      if (qty === 2){
        state[otherMap] = new Map({
          ...
          layers: [
            ...
            ...otherLayersData.map((node) =>
                getMapLayers.getLayer(
                    state.layers[node.nodeIndex].group_layers[node.subNodeIndex],
                    node.nestedSubNodeIndex,
                    node.id,
                ),
            ),
          ],
    ...
    function changeTarget(value){
      ...
      if (state.mapCount === 1){
        // 目標地圖為空
        if (!state[`map${value}`]){
          let otherLayers = state[`map${value}LayerStatus`].filter(
              (node) => node !== '3D',
          )
          // TODO: 優化，把node0_subNode4_nestedSubNodeundefined移到最後面
          if (otherLayers.includes('node0_subNode4_nestedSubNodeundefined')){
            let a = otherLayers.filter(
                (node) => node !== 'node0_subNode4_nestedSubNodeundefined',
            )
          ...
          state[`map${value}`] = new Map({
            target: `map${value}`,
            layers: [
              baseMapList.getBaseMapData(
                  state.temp[`map${state.targetNum}BaseStatus`],
              ),
              ...otherLayersData.map((node) => {
                return getMapLayers.getLayer(
                    state.layers[node.nodeIndex].group_layers[node.subNodeIndex],
                    node.nestedSubNodeIndex,
                    node.id,
                )
    ...
    function mapClickEvent(target){
      ...
      selector.on('select', (event) => {
        ...
        if (selectedFeatures){
          ...
          let selectIds = (
              selectedFeatures.getId() ?? selectedFeatures.getGeometryName()
          ).split('.')
...
<template>
  <div>
    <!-- TODO: 寬度設定是否調整 -->
    <div
        class="w-100 d-flex justify-content-between flex-sm-row flex-wrap flex-sm-nowrap mapWrap"
        id="mapWrap"
    >
      <div
          id="map1"
    ...
    <asideTool
        class="asideTool position-absolute top-50 translate-middle-y"
        id="asideTool"
        :onChangeTarget="
        (value) => {
          changeTarget(value);
    ...
    />
    <div
        class="SearchBar d-block d-sm-block position-fixed w-100 w-sm-auto position-sm-absolute p-3 p-sm-0"
    >
      <div
      ...
      >
        <img
            src="@/assets/logo.svg"
            alt=""
            class="logo col-5 col-sm-auto me-0 me-sm-5"
        />
        <mapSourceOption
            class="mapSourceOption col-5 col-sm-auto d-block d-sm-block"
    ...
    <div class="conditionCom d-none d-sm-block position-absolute">
      <div class="mb-4">
        <button
            class="border-0 w-100 rounded-4 bg-steel text-white text-center p-2 fw-bold fs-5"
        ...
        <OverLayer
            :text="'可選擇要加入的圖層'"
            :styles="'right: 105%;top: 0;text-align: right;'"
        />
      </div>
    ...
    <div
        class="lightWrap w-100 h-100 d-flex justify-content-center align-items-center"
        v-if="state.deleteLightbox"
    >
      <div class="p-4 rounded bg-white" style="width: 250px">
        <p class="text-center fw-bold">是否要取消全部圖層</p>
        <div class="d-flex justify-content-around">
          <button
              class="rounded px-3 py-1 bg-steel text-white border-0"
              @click="
              () => {
                layerControl({
          ...
          <button
              class="rounded px-3 py-1 bg-secondary bg-gradient text-white border-0"
              @click="
              () => {
                state.deleteLightbox = false;
              }
    ...
    <div
        id="popup"
        class="popup position-absolute bottom-0"
        :ref="
        (e) => {
    ...
    <div
        class="stepOverLayer position-absolute top-0 start-0 w-100 h-100 bg-black opacity-50"
        id="firstEnter"
